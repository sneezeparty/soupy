...
db_dir = os.getenv(
    "SOUPY_DB_DIR",
    os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "soupy_database", "databases"),
)
...
try:
    ...
    for msg in all_messages:
        ...
        if last_time is None:
            ...
            if (
                time_diff <= timedelta(minutes=time_window_minutes)
                and msg["channel_name"] == current_conversation[0]["channel_name"]
            ):
                # Same conversation
    ...
    print(
        f"Found {len(conversations)} conversation(s) with 2+ messages and 2+ users (within {time_window_minutes} minutes)"
    )
    print()
    ...
    if conversations:
        ...
        print(
            f"⏰ {first_time.strftime('%Y-%m-%d %H:%M:%S')} to {last_time.strftime('%Y-%m-%d %H:%M:%S')} ({duration})"
        )
        print()
