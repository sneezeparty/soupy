...
class ImageSearchCog(commands.Cog):
    ...
    async def image_search_command(self, interaction: discord.Interaction, query: str):
        ...
        try:
            # Get image search results using context-managed client to avoid socket leaks
            def _run_images(q: str) -> List[Dict]:
                with DDGS() as ddg:
            ...
            if source_url:
                embed.add_field(name="Source", value=f"[{title}]({source_url})", inline=False)
