...
DB_DIR = settings.soupy_db_dir or os.path.join(os.path.dirname(__file__), "databases")
...
def get_stats(guild_id: int) -> Dict:
    ...
    archive_scan_interval_minutes = int(guild_name_row["archive_scan_interval_minutes"] or 0) if guild_name_row else 0
...
def setup_scan_command(bot, owner_ids: list):
    ...
    async def soupyscan_command(interaction: discord.Interaction):
        ...
        scan_task = asyncio.create_task(run_scan_background(interaction, guild_id, guild_name))
...
async def run_scan_background(interaction: discord.Interaction, guild_id: int, guild_name: str):
    ...
    try:
        ...
        if is_first_scan:
            ...
            try:
                lookback_days = settings.first_scan_lookback_days
            except ValueError:
                lookback_days = 365
            if lookback_days < 1:
                lookback_days = 1
        ...
        excluded_channel_ids_str = ",".join(str(x) for x in settings.scan_exclude_channel_ids)
        excluded_channel_ids = []
        if excluded_channel_ids_str:
            excluded_channel_ids = [
                int(cid.strip()) for cid in excluded_channel_ids_str.split(",") if cid.strip().isdigit()
            ]
        ...
        for channel_idx, channel in enumerate(channels, 1):
            try:
                ...
                try:
                    ...
                    async for message in channel.history(limit=None, after=after_time, oldest_first=True):
                        ...
                        show_preview = (
                            ...
                            or (
                                message.attachments
                                and any(not att.filename.lower().endswith(".gif") for att in message.attachments)
                            )
                        )
                                ...
                                if any(filename.endswith(ext) for ext in [".png", ".jpg", ".jpeg", ".webp"]):
                                    try:
                                        ...
                                        if desc:
                                            ...
                                            logger.debug(
                                                f"  → Image description returned None for {attachment.filename}"
                                            )
                        ...
                        if unique_urls:
                            ...
                            if any(
                                ...
                                for ext in [".gif", ".jpg", ".jpeg", ".png", ".webp", ".bmp", ".svg"]
                            ):
                                total_urls_skipped += 1
                                ...
                                try:
                                    ...
                                    async with aiohttp.ClientSession() as session:
                                        ...
                                        if content:
                                            ...
                                            model_name = settings.local_chat
                                            if model_name:
                        ...
                        if messages_processed % 25 == 0:
                            ...
                            logger.info(
                                f"  📊 [{channel_idx}/{total_channels}] Progress: {messages_processed} scanned, {messages_added} new | Total: {total_messages_scanned} scanned, {total_messages_added} new | Images: {total_images_processed} | URLs: {total_urls_processed} | Rate: {rate:.1f} msg/sec"
                            )
                ...
                logger.info(
                    f"  ✅ #{channel_name}: {messages_processed} scanned, {messages_added} new in {channel_elapsed:.1f}s"
                )
...
async def trigger_scan_programmatic(bot, guild_id: int) -> tuple[bool, str]:
    ...
    try:
        ...
        # Create background task for the scan
        scan_task = asyncio.create_task(run_scan_background(mock_interaction, guild_id, guild_name))
        active_scans[guild_id] = scan_task
